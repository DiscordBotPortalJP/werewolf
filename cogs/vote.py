...
class Vote(commands.Cog):
    ...
    async def change_date(self, ctx):
        ...
        await ctx.send(f'投票の結果 {executed.id} さんが処刑されました')

        players = player.alive_players(self.bot.players)
        raided = player.raided(players)
        if raided is not None:
            await ctx.send(f'{raided.id} さんが無残な姿で発見されました')

        if is_village_win(players):
            self.bot.game_status = 'nothing'
            await ctx.send('ゲームが終了しました。人狼が全滅したため村人陣営の勝利です!')
            return

        if is_werewolf_win(players):
            self.bot.game_status = 'nothing'
            await ctx.send('ゲームが終了しました。村人陣営の数が人狼陣営の数以下になったため人狼陣営の勝利です!')
            return

        players = player.alive_players(self.bot.players)
        fortuned = player.fortune(players)
        if fortuned is not None:
            text = f'{fortuned.id} さんの占い結果はxx陣営です。'
            player.get_fortuneteller(players).send(text)
